...
class _RunBase(ChainedIdentity, HasRunPortal):
    ...
    def __init__(self, experiment, run_id, outputs=None, logs=None,
                 _run_dto=None, _worker_pool=None,
                 _user_agent=None, _ident=None,
        ...
        self._client = RunHistoryFacade(self._experiment, self._run_id, RUN_ORIGIN, run_dto=_run_dto,
                                        worker_pool=_worker_pool, user_agent=user_agent, _parent_logger=self._logger,
        ...
        self._context_manager = RunContextManager(self, outputs=outputs, logs=logs,
                                                  heartbeat_enabled=_run_dto_as_dict.get("heartbeat_enabled", False),
                                                  _parent_logger=self._logger, py_wd=py_wd, deny_list=deny_list,
    ...
    def __enter__(self):
        if self._outputs is not None and not os.path.exists(self._outputs):
            os.mkdir(self._outputs)
