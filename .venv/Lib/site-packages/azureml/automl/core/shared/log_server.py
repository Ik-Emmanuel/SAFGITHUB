...
logger_names = collections.defaultdict(threading.Condition)
        ...
        with lock:
            ...
            if DEBUG_MODE:
                try:
                    ...
                    # Notify threads waiting on condition variables to facilitate waiting for log records during tests
                    cv = logger_names[record.name]
                    with cv:
                        cv.notify_all()
                except Exception:
                    pass
...
def install_sockethandler(name: str,
    ...
    logger_names[name] = threading.Condition()
...
def _wait_for_log(logger_name: str, timeout: Optional[float] = 60) -> None:
    """
    ...
    cv = logger_names[logger_name]

    # 0. Caller acquired server lock. Server will block when it attempts to handle log records.

    # 1. Lock the condition variable. Server will block when it attempts to notify other threads.
    with cv:
        # 2. Release the server lock so it can start handling records again.
        lock.release()

        # 3. Release the condition variable and then sleep this thread until the server wakes it up.
        # The server can once again notify other threads.
        cv.wait(timeout=timeout)

        # 4. Reacquire the condition variable (happens automatically after this thread wakes up).

        # 5. Reacquire the lock since it was acquired before calling this function.
        lock.acquire()
    # 6. Release the condition variable.
