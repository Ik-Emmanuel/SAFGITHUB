...
class PythonFS(WorkingDirectoryFS, TrackableFS):
    ...
    def track(self, run_tracker, track_folders, blacklist):
        ...
        for path in track_folders:
            if os.path.exists(path):
                ...
                try:
                    self._upload_folder(run_tracker, path, blacklist)
    ...
    def _upload_folder(self, run_tracker, path, blacklist):
        with PythonFS._lock:
            for pathl, _subdirs, files in os.walk(path):
                ...
                if paths_to_upload:
                    run_tracker._client.artifacts.upload_files(paths_to_upload,
                                                               RUN_ORIGIN,
                                                               run_tracker._client._data_container_id)
...
class SparkDFS(WorkingDirectoryFS, TrackableFS):
    ...
    def _upload_hdi_outputs(self, run_tracker):
        import timeit
        ...
        if not force_upload_by_HDFS and is_wasb:
            ...
            self.logger.debug("Tracking outputs in {0}/{1} with prefix {2}".format(container_path,
                                                                                   run_prefix,
                                                                                   track_prefix))
            run_tracker._client.artifacts.batch_ingest_from_sas(RUN_ORIGIN,
                                                                run_tracker._client._data_container_id,
                                                                container_sas,
                                                                container_path,
                                                                run_prefix,
