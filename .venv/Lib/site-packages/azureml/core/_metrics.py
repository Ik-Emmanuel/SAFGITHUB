...
class TableMetric(InlineMetric):
    ...
    def create_v2_dto(self):
        ...
        columns = {}
        for key in keys:
            value = TableMetric._get_value_from_column(self.value[key])
            _, metric_value_type = Metric.get_converted_value_and_type_v2(value)
            columns[key] = metric_value_type
        reference_column = self.value[list(keys)[0]]
        column_length = len(reference_column)
        metric_values = []
        created_utc = get_timestamp()
        for i in range(column_length):
            metric_values.append(TableMetric._get_metric_v2_value_from_row(self.value, created_utc, position=i))
